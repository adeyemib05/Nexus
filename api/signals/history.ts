...
import { getHistoricalSignals } from '../db';
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const signals = await getHistoricalSignals(symbol, limit, from, to);
    ...
    console.error('[Signals History API] Error:', err);
    return res.status(500).json({
      success: false,
      error: err.message || 'Failed to retrieve signal telemetry history',
      timestamp: Date.now(),
    });
  }
}
